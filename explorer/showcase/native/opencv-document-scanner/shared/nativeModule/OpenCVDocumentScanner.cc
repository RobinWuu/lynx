...
void SetFunction(napi_env env, napi_value object, const char* name,
                 napi_callback callback) {
  napi_value function;
  Check(env, napi_create_function(env, name, NAPI_AUTO_LENGTH, callback,
                                  nullptr, &function));
...
std::vector<unsigned char> EncodeJpeg(const cv::Mat& image, int quality) {
  ...
  const int clamped_quality = std::max(40, std::min(quality, 95));
  if (!cv::imencode(".jpg", image, bytes,
                    {cv::IMWRITE_JPEG_QUALITY, clamped_quality})) {
...
Napi::Value GetRuntimeInfo(const Napi::CallbackInfo& info) {
  ...
  stream << " / Android";
#elif defined(LYNX_LIBRARY_HARMONY) || defined(__OHOS__)
  stream << " / HarmonyOS";
...
napi_value GetRuntimeInfoCallback(napi_env env, napi_callback_info info) {
  return GetRuntimeInfo(Napi::CallbackInfo(env, info));
}

napi_value ScanDocumentCallback(napi_env env, napi_callback_info info) {
  return ScanDocument(Napi::CallbackInfo(env, info));
}

void BindOpenCVDocumentScanner(napi_env env, napi_value exports) {
  SetFunction(env, exports, "getRuntimeInfo", GetRuntimeInfoCallback);
  SetFunction(env, exports, "scanDocument", ScanDocumentCallback);
}

static napi_value CreateOpenCVDocumentScanner(napi_env env,
                                              napi_value exports) {
  BindOpenCVDocumentScanner(env, exports);
  return exports;
}
...
extern "C" napi_value LynxAutolinkCreateOpenCVDocumentScanner(
  ...
  (void)module_name;
  (void)opaque;
  return CreateOpenCVDocumentScanner(env, exports);
}
