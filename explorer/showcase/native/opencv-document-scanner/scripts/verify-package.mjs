...
const lynxtronRegistration = fs.readFileSync(
  path.join(packageRoot, 'lynxtron', 'generated_napi_registration.cc'),
  'utf8',
);
if (
  !lynxtronRegistration.includes(
    '_napi_register_xx_OpenCVDocumentScanner();',
  ) ||
  lynxtronRegistration.includes('lynx_env_register_native_module')
) {
  fail(
    'Lynxtron NAPI registration must use standard napi_module_register instead of the platform native-module registry.',
  );
}
