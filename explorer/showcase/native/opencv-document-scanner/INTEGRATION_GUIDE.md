...
> [!WARNING]
> 本文是早期接入过程记录，其中的旧 manifest、三端目录、Lynxtron 双 entry、
> OpenCV 5 产物和验证结论已经过时。
>
> 当前四端接入、发布内容和新宿主步骤以 package 根目录的
> [`README.md`](./README.md) 为准。最终实现选择独立的
> `napi-native-module` feature，共享
> `shared/nativeModule/OpenCVDocumentScanner.cc`，注册代码归 codegen 所有，
> 页面统一通过 `NativeModules.OpenCVDocumentScanner` 调用。
...
2. include `:lynx_library__byted_lynx_opencv_document_scanner`。
...
cd explorer/android
./gradlew \
  :lynx_library__byted_lynx_opencv_document_scanner:assembleDebug \
  :LynxExplorer:assembleWithoutSparklingNoasanDebug \
  --console=plain \
  --no-daemon
```
...
```text
explorer/android/lynx_explorer/build/outputs/apk/withoutSparklingNoasan/debug/
  LynxExplorer-withoutSparkling-noasan-debug.apk
```
