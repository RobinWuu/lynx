...
> [!WARNING]
> 本文是早期原型的开发记录，包含旧的三端目录、手写注册代码和已废弃的
> `napi-native-module` 模型，不能作为当前实现的操作指南。
>
> 当前实现以 package 根目录的 [`README.md`](./README.md) 为准：
>
> - public feature 为 `native-module`、`element`、`service`；
> - 共享实现由 `native-module-backend=node-api` 选择；
