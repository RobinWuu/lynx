...
> [!WARNING]
> 本文是早期原型的开发记录，包含旧的三端目录和手写注册代码，不能作为当前实现的
> 操作指南。
>
> 当前实现以 package 根目录的 [`README.md`](./README.md) 为准：
>
> - `native-module` 与 `napi-native-module` 是两个独立 feature；
> - scanner 只选择 `napi-native-module`；
> - Android、iOS、HarmonyOS、Lynxtron 共用
>   `shared/nativeModule/OpenCVDocumentScanner.cc`；
> - 注册代码由 codegen 写入
>   `shared/nativeModule/generated/OpenCVDocumentScannerRegistration.cc`；
> - 页面统一先 import package root，再通过
>   `NativeModules.OpenCVDocumentScanner` 调用；
> - 不要复制本文中的 `LYNX_NAPI_MODULE`、`NAPI_MODULE`、手写 Lynxtron entry
>   或旧 manifest 示例。
...
`LYNX_LIBRARY_USE_PRIMJS_NAPI_MODULE` 生成标准
`napi_module`/`napi_module_register` 注册入口，不改变业务方法使用的标准
Node-API。移动端可由 constructor 或宿主生成代码触发 `_napi_register_xx_*`；
Lynxtron 的 package entry 显式触发同一个注册符号，见第 8 节。
